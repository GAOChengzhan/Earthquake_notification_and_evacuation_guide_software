...
private void Form5_Load(object sender, EventArgs e)
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string search1 = "select left(F1,10) as 日期,count(F6) as 发生次数 from Table1$ where F1>= " +"'" + dateTimePicker1.Value.ToString("yyyy-MM-dd")+ "'"+"AND F1<= " +"'" +dateTimePicker2.Value.ToString("yyyy-MM-dd ")+"' group by left(F1,10)";  
            ...
            {
                string search2 = "select left(F1,4) as 日期,count(F6) as 发生次数 from Table1$ where F1>= " + "'" + dateTimePicker1.Value.ToString("yyyy-MM-dd") + "'" + "AND F1<= " + "'" + dateTimePicker2.Value.ToString("yyyy-MM-dd ") + "' group by left(F1,4)";
            ...
            {
                string search2 = "select left(F1,7) as 日期,count(F6) as 发生次数 from Table1$ where F1>= " + "'" + dateTimePicker1.Value.ToString("yyyy-MM-dd") + "'" + "AND F1<= " + "'" + dateTimePicker2.Value.ToString("yyyy-MM-dd ") + "' group by left(F1,7)";
                SqlDataAdapter myadapter2 = new SqlDataAdapter(search2, mycon);
                mytable1.Clear();
                myadapter2.Fill(mytable1);
            }
...
private void button2_Click(object sender, EventArgs e)
        {
            ...
            string search2 = "select count(case when F5<60 then F5 end) as 浅源地震,count(case when F5<300 and F5>60 then F5 end) as 中源地震,count(case when F5>300 then F5 end) as 深源地震 from Table1$ where F1>= '" + dateTimePicker1.Value.ToString("yyyy-MM-dd") + "' AND F1<= " + "'" + dateTimePicker2.Value.ToString("yyyy-MM-dd ") + "'";
